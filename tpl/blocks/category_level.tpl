...
<div>
<select size="10" {if $section.Key}class="section_{$section.Key}"{/if} {if $category.ID}id="tree_area_{$category.ID}"{/if}>
	...
	{if $category && $listing_types[$cat_type].Cat_custom_adding && $category.Add == 1}
		...
		<span class="tmp-category">
			...
			<span class="tmp_input hide">
				<input type="text" />
				<input value="{$lang.add}" onclick="xajax_addTmpCategoryTpl($(this).prev().val(), '{$category.ID}');$(this).val('{$lang.loading}');" type="button" />
