		...
		<div class="submit-cell clearfix">
				...
				{if ($field.Multilingual && $languages|@count > 1) || is_array($fVal.$fKey)}
					...
					<div class="ml_tabs_content light-inputs">
						...
						<div lang="{$language.Code}" {if !$smarty.foreach.langF.first}class="hide"{/if}>
							<input type="text" name="f[{$field.Key}][{$language.Code}]" maxlength="{if $field.Values != ''}{$field.Values}{else}255{/if}" {if $fVal.$fKey[$language.Code]}value="{$fVal.$fKey[$language.Code]}"{elseif $field.Default}value="{$lang[$field.pDefault]}"{/if} />
						</div>
						{/foreach}
					</div>
				{else}
					<input type="text" name="f[{$field.Key}]" {if $field.Values < 100}size="{$field.Values}" class="wauto"{/if} maxlength="{if $field.Values != ''}{$field.Values}{else}255{/if}" {if $fVal.$fKey}value="{$fVal.$fKey}"{elseif $field.Default}value="{$lang[$field.pDefault]}"{/if} />
				{/if}
			...
			{elseif $field.Type == 'date'}
				...
				{elseif $field.Default == 'multi'}
					<input placeholder="{$lang.from}" class="date" type="text" id="date_{$field.Key}_from" name="f[{$field.Key}][from]" maxlength="10" value="{$fVal.$fKey.from}" />
					<input placeholder="{$lang.to}" class="date" type="text" id="date_{$field.Key}_to" name="f[{$field.Key}][to]" maxlength="10" value="{$fVal.$fKey.to}" />
			...
			{elseif $field.Type == 'select'}
				...
				<select {if $field.Key == 'year'}class="w120"{/if} name="f[{$field.Key}]">
				...
				{foreach from=$field.Values item='option' key='key' name='checkboxF'}
					...
					<span class="custom-input">
						<label title="{$lang[$option.pName]}">
							<input type="checkbox" {if is_array($fVal.$fKey)}{foreach from=$fVal.$fKey item='chVals'}{if $chVals == $key}checked="checked"{/if}{/foreach}{else}{foreach from=$field.Default item='chDef'}{if $chDef == $key && !empty($chDef)}checked="checked"{/if}{/foreach}{/if} value="{$key}" name="f[{$field.Key}][{$key}]" />
							{$lang[$option.pName]}
						</label>
					</span>
				{/foreach}
			...
			{elseif $field.Type == 'radio'}
				...
				{foreach from=$field.Values item='option' key='key' name='checkboxF'}
					...
					<span class="custom-input">
						<label title="{$lang[$option.pName]}">
							<input type="radio" value="{$key}" name="f[{$field.Key}]" {if $fVal.$fKey}{if $fVal.$fKey == $key}checked="checked"{/if}{else}{if $field.Default == $key}checked="checked"{/if}{/if} />
							{$lang[$option.pName]}
						</label>
					</span>
				{/foreach}
