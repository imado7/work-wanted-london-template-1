...
<ul class="tabs">
	{if $search_forms|@count > 0}
		{foreach from=$search_forms item='search_form' key='sf_key' name='sformsF'}
			...
			<li {if $smarty.foreach.sformsF.first}class="{if $smarty.foreach.sformsF.first && !$keyword_search}active{/if}"{/if} id="tab_{$sf_key|replace:'_':''}">{$lang[$tab_phrase]}</li>
		{/foreach}
	{/if}
	{assign var='ks_phrase' value='blocks+name+keyword_search'}
	<li {if $keyword_search || !$search_forms}class="active"{/if} id="tab_keyword">{$lang.$ks_phrase}</li>
</ul>
<!-- tabs end -->
