...
if ( !$request_item || !$request_mode || !in_array($request_mode, array('photo', 'listing', 'thumbnail', 'category')) )
	exit;
	...
	case 'thumbnail':
		...
		if ( $account_id ) {
			...
			if ( $thumbnail_name = $rlActions -> upload('thumbnail', $thumbnail_name, 'C', array(200, 100), false, false) ) {
				...
				if ( $current_thumbnail = $reefless -> getOne('Photo', "`ID` = '{$account_id}'", 'accounts') ) {
