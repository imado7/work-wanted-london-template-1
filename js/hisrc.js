 ...
 * 
 * IT IS SOURCE OF THE CODE, THE COMPRESSED COPY ALREADY CONTAINS IN THE TEMPLATE LIB.JS FILE
 *
 */
				...
				$el.on('speedTestComplete.hisrc', function(){
					if (speedConnectionStatus === STATUS_COMPLETE) {
						if (isSlowConnection) {
							...
							if ( $.browser.msie && $.browser.version <= 8 ) {
								$el.css('filter', "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + $el.data('m1src') + "', sizingMethod='scale')");
							}
						} else {
